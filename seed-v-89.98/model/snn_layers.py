...
class quat_threshold(torch.autograd.Function):
    """
    Quaternion modulus-based spike trigger:
      spike = H(||q|| - v_th)
    Surrogate gradient: Gaussian around (||q|| - v_th)
    """

    @staticmethod
    def forward(ctx, q, sigma, v_th):
        # q: (..., 4)
        # sigma: float / tensor scalar
        # v_th: float / tensor scalar
        q_norm = torch.linalg.vector_norm(q, dim=-1)  # (...,)

        # Heaviside step
        out = (q_norm - v_th >= 0).to(q.dtype)

        ctx.save_for_backward(q, q_norm)
        ctx.sigma = sigma
        ctx.v_th = v_th
        return out
    ...
    @staticmethod
    def backward(ctx, grad_output):
        q, q_norm = ctx.saved_tensors
        sigma = ctx.sigma
        v_th = ctx.v_th

        # surrogate grad wrt (q_norm - v_th)
        x = q_norm - v_th                       # (...,)
        exp_term = torch.exp(-(x * x) / (2.0 * sigma**2))
        g = exp_term / (2.506628 * sigma)       # (...,)

        # chain rule: d||q||/dq = q / (||q|| + eps)
        eps = 1e-8
        dqnorm_dq = q / (q_norm.unsqueeze(-1) + eps)  # (..., 4)

        # grad_output: (...,) -> (...,1)
        grad_q = (grad_output * g).unsqueeze(-1) * dqnorm_dq
        return grad_q, None, None


class quat_threshold_layer(torch.nn.Module):
    def __init__(self, sigma=0.4, v_th=1.0):
        super().__init__()
        self.sigma = torch.tensor(float(sigma))
        self.v_th = torch.tensor(float(v_th))

    def forward(self, q):
        return quat_threshold.apply(q, self.sigma.to(q.device), self.v_th.to(q.device))
...
class first_order_low_pass_layer(torch.nn.Module):
    ...
    def forward(self, input_spikes, states):
        ...
        # unbind along last dimension
        inputs = input_spikes # list(input_spikes.unbind(dim=-1))
        spikes = []
        length = len(inputs)

       # for i in range(length):
        spike, states = self.first_order_low_pass_cell(inputs, states)
            #spikes += [spike]
        return spike, states
...
class neuron_cell(torch.nn.Module):
    def __init__(self, input_size, neuron_number, step_num, batch_size, tau_m,
        ...
        if self.input_type != 'axon':
            ...
            self.weight = QuaternionLinear(input_size, neuron_number, bias=train_bias)
        self.tau_m = torch.full((neuron_number,), tau_m, dtype=float)

        self.sigma = torch.nn.Parameter(torch.tensor(0.4))
        self.sigma.requires_grad = False
        self.v_th = torch.nn.Parameter(torch.tensor(float(0.4)))   # ➕ ADDED
        self.v_th.requires_grad = False                             # ➕ ADDED
    ...
    def forward(self, current_input, prev_states):
        ...
        threshold_function = threshold.apply
        spike = threshold_function(current_v, self.sigma)
        #spike = quat_threshold.apply(current_v, self.sigma.to(current_v.device), self.v_th.to(current_v.device))
...
class neuron_layer(torch.nn.Module):
    ...
    def forward(self, input_spikes, states):
        ...
        spikes = []
        # for i in range(len(inputs)):
        spike, states = self.neuron_cell(inputs.float(), states)
            # spikes += [spike]
        return spike, states
    ...
    def create_init_states(self):
        ...
        init_v = torch.zeros(self.neuron_number).float().to(device)
        init_reset_v = torch.zeros(self.neuron_number).float().to(device)
