...
class QLIF_Model(nn.Module):
    def __init__(self, args):
        ...
        self.axon1 = first_order_low_pass_layer((args.dim_1,), args.length, self.batchsize, args.tau_m,
                                                True)
        self.snn1 = neuron_layer(args.dim_1, 512, args.length, self.batchsize, args.tau_m, True,
                                 True)

        self.axon2 = first_order_low_pass_layer((512,), args.length, self.batchsize, args.tau_m,
                                                True)
        self.snn2 = neuron_layer(512, 128, args.length, self.batchsize, args.tau_m, True,
                                 True)
        ...
        self.radius = 10
        self.fc2 = SLR_layer(512, args.n_classes)
    ...
    def forward(self, inputs):
        ...
        feat = self.radius * feat / (torch.norm(feat, dim=1, keepdim=True) + 1e-10)  # 归一化
        return feat
...
class ResidualAdd(nn.Module):
    def __init__(self, fn, downsample=None):
        super().__init__()
        self.fn = fn
        self.downsample = downsample  # 用于调整残差分支的维度

    def forward(self, x, **kwargs):
        res = x
        x = self.fn(x, **kwargs)  # 主分支输出：[batch, 128]
        if self.downsample is not None:
            res = self.downsample(res)  # 调整残差分支维度到 [batch, 128]
        x += res
        return x
...
class EEGQ_S():
    def __init__(self, args, nsub, fold):
        ...
        self.radius = 10
        self.start_epoch = 0
        self.root = '/home/lyc/research/research_5/DA/ExtractedQuantFeatures/'
    ...
    def test_suda(self, loader, model):
        ...
        with torch.no_grad():
            ...
            for i in range(len(loader['test'])):
                ...
                inputs = inputs.view(inputs.size(0), inputs.size(1), -1)  #  [b，3，310]
    ...
    def train(self, fold):
        ...
        for e in range(self.n_epochs):
            ...
            for i, data in enumerate(train_dataset):
                ...
                features_s_Adver = Adver_network.ReverseLayerF.apply(tok, lambda_adv)#GRL
            ...
            if (e + 1) % 1 == 0:
                ...
                with torch.no_grad():        
                    ...
                    for batch_idx, tar_data in enumerate(test_dataset):
                        ...
                        Ty = tar_data['Ty']
                        Tx = Tx.float().cuda()
                        Tx = Tx.view(Tx.size(0), Tx.size(1), -1)  # 
                        # Tx = Tx.permute(0, 2, 1) 
    ...
    def fine_tuning(self, args, X, Y, model):
        ...
        for i in range(args.max_iter2):
            ...
            inputs_source = inputs_source.view(inputs_source.size(0), inputs_source.size(1), -1)  #  62×5=310 [b，3，310]
            # inputs_source = inputs_source.permute(0, 2, 1) 
            inputs_target = inputs_target.view(inputs_target.size(0), inputs_target.size(1), -1)  #  62×5=310 [b，3，310]
...
def main(args):
    pre_train = []
    tuning = []
    result_write = open("/home/lyc/research/research_6/DA-93_24-2\snapshot.txt", "w")
    total_acc = []
    for i in range(15):
        args.target = 15 - i
        ...
        exgan = EEGQ_S(args, i + 1, 1)
        
        ba, aa, _, _, X, Y, model, epochs_acc  = exgan.train(1)
        total_acc.append(epochs_acc)
        # save_path = f"/home/lyc/research/research_7/Base-9275-921/best_model_subjects_{i}_state.pth"
        # torch.save(model.state_dict(), save_path)
        # load_path = f"/home/lyc/research/research_7/SNN-9212-NCE/best_model_subjects_{i}_state.pth"
        # model = ViT(emb_size=310, depth=6, bottleneck_dim=128, n_classes=3).float().cuda()
    ...
    print(f"all epochs mean acc: {epoch_mean_acc}")
    best_epoch = np.argmax(epoch_mean_acc) + 1 
    best_epoch_acc = epoch_mean_acc[best_epoch - 1]
    print(f"\nBest epoch: {best_epoch}，mean acc = {best_epoch_acc:.2f}%")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--file_path', type=str, default='/home/lyc/research/research_5/DA/ExtractedQuantFeatures/', help="Path from the current dataset")
    parser.add_argument('--log_file')
    parser.add_argument('--n_classes', type=int, default=3)
    parser.add_argument('--d_classes', type=int, default=14)
    ...
    parser.add_argument('--dim_1', type=int, default=3720)
