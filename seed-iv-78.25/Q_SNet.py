...
import time
import scipy.io
from modules import PseudoLabeledData, load_seed, load_seed_raw, fine_tuning_load_XY,load_seed_three_feature, load_seed_iv
...
class MultiHeadAttention(nn.Module):
    def __init__(self, emb_size, num_heads, dropout):
        ...
        self.keys = QuaternionLinearAutograd(620, 620, bias=False,
                 init_criterion='glorot', weight_init='quaternion',
                 seed=None, rotation=True, quaternion_format=True, scale=False)
        self.queries = QuaternionLinear(620, 620)
        self.values = QuaternionLinear(620, 620)
...
class ExGAN():
    def __init__(self, args, nsub, fold):
        ...
        self.model = Q_SNet(emb_size=620, depth=6, bottleneck_dim=128, n_classes=4).float().cuda()
    ...
    def get_source_data(self, feature="de_LDS"):
        if self.args.dataset == "seed":
            ...
            train_dataset, test_dataset, X, Y = load_seed_iv(self.args, self.args.file_path, session=self.args.session)
    ...
    def fine_tuning(self, args, X, Y, model):
        ...
        for i in range(args.max_iter2):
            ...
            all_labels = torch.cat([labels_source, torch.argmax(pre_target, dim=1)], dim=0)  # 拼接标签
...
def main(args):
    ...
    for i in range(15):
        ...
        print(f"Q-SNet train time: {time_cost:.4f} 秒")
        # 计算参数总数
    ...
    result_write.close()
    save_path = "/home/lyc/research/research_6/for-discussion/SEED-IV"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset',type=str,default='seed-iv')
    ...
    parser.add_argument('--num_class',type=int,default=4,help='the number of classes')
    ...
    parser.add_argument('--file_path', type=str, default='/home/lyc/research/research_5/DA/eeg_feature_smooth/', help="Path from the current dataset")
    ...
    parser.add_argument('--dim_1', type=int, default=5580)
